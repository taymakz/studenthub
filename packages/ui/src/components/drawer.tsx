...
function Drawer({
  ...
  // Seed from the ambient <html dir> (mirrors <Dialog>) so controlled
  // drawers without a trigger - nothing calls setDir - still render RTL;
  ...
  return (
    <DrawerPrimitive.Viewport
      ...
      className={cn(
        ...
        position === "bottom" &&
          "flex items-end justify-center pt-[calc(3rem+var(--tg-safe-area-inset-top,0px)+var(--tg-content-safe-area-inset-top,0px))]",
        ...
        // Bottom edge gets a physical gap at every breakpoint (pb-2 on
        // mobile, the desktop-only --inset at sm+).
        variant === "inset" && "px-(--inset) [--inset:--spacing(4)]",
        variant === "inset" && position !== "bottom" && "pt-(--inset)",
        variant === "inset" && position !== "top" && "pb-(--inset) pb-2",
        className
      )}
      {...props}
    />
          ...
          <DrawerPrimitive.Popup
            ...
            className={cn(
              ...
              (position === "bottom" || position === "top") &&
                ...
                "h-(--drawer-height,auto) max-h-[calc(100dvh-(--spacing(12))-var(--tg-safe-area-inset-top,0px)-var(--tg-content-safe-area-inset-top,0px))] [--height:max(0px,calc(var(--drawer-frontmost-height,var(--drawer-height))))] data-nested-drawer-open:h-(--height)",
              position === "bottom" &&
...
function DrawerHeader({
  ...
  const defaultProps = {
    className: cn(
      "flex flex-col gap-2 p-6 text-center in-[[data-slot=drawer-popup]:has([data-slot=drawer-panel])]:pb-3 max-sm:pb-4",
...
function DrawerPanel({
  ...
  const defaultProps = {
    className: cn(
      "p-6 pb-[calc(2rem+env(safe-area-inset-bottom,0px))] in-[[data-slot=drawer-popup]:has([data-slot=drawer-footer]:not(.border-t))]:pb-1 in-[[data-slot=drawer-popup]:has([data-slot=drawer-header])]:pt-1",
      !allowSelection && "cursor-default",
      className
    ),
