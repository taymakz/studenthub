...
const heatmapTooltipDateFmt = new Intl.DateTimeFormat("fa-IR", {
  day: "numeric",
  month: "long",
  year: "numeric",
})

const heatmapTooltipWeekdayFmt = new Intl.DateTimeFormat("fa-IR", {
  weekday: "long",
})

/** Tooltip header date — e.g. `۳ شهریور ۱۴۰۵`. */
export function formatHeatmapTooltipDate(date: Date): string {
  return heatmapTooltipDateFmt.format(date)
}

/** Tooltip weekday line — e.g. `سه‌شنبه`. */
export function formatHeatmapTooltipWeekday(date: Date): string {
  return heatmapTooltipWeekdayFmt.format(date)
}

/** Tooltip contribution line — e.g. `۴ سفارش`. */
export function formatHeatmapContributionLabel(
  count: number,
  _date?: Date
): string {
  const faCount = new Intl.NumberFormat("fa-IR").format(count)
  return `${faCount} سفارش`
}

/** Sunday-first day labels for heatmap row bins. */
export const HEATMAP_DAY_LABELS = ["ی", "د", "س", "چ", "پ", "ج", "ش"] as const
