...
export function ElasticSlider({
  ...
  /**
   * Reads the ambient text direction where the slider actually renders, since
   * this project is RTL-first by default. Value 0 always maps to the reading
   * start (right edge in RTL) and value max to the reading end, matching how
   * a native <input type="range"> behaves in RTL browsers.
   */
  ...
  const handleInsetStart = useTransform(
    fillPercent,
  ...
  const positionToValue = useCallback(
    (clientX: number) => {
      ...
      let percent = clamp(sceneX / nativeWidth, 0, 1)
      if (dirRef.current === "rtl") percent = 1 - percent
  ...
  const handleKeyDown = useCallback(
    (e: React.KeyboardEvent) => {
      ...
      // Right/Left arrows follow reading direction: in RTL, ArrowRight moves
      // toward the reading start (decreases value) like a native range input.
    ...
    >
      ...
      >
        ...
        >
          {Array.from({ length: hashMarkCount }, (_, i) => (
            <div
              ...
              style={{
                insetInlineStart: `${hashMarkPct(i)}%`,
        ...
        <motion.div
          ...
          style={{ insetInlineStart: handleInsetStart, y: "-50%" }}
          animate={{
