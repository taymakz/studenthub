...
/** Column labels use the Jalali month of each week's first day (fa-IR).
    The Jalali year is appended on the first label and whenever the graph
    crosses into a new year, so the opening «شهریور ۱۴۰۴» is not mistaken
    for the current month. */
const jalaliMonthFmt = new Intl.DateTimeFormat("fa-IR", {
  month: "long",
  year: "numeric",
})
...
const getMonthLabels = (weeks: Week[]): MonthLabel[] => {
  ...
  return weeks
    .reduce<MonthLabel[]>((labels, week, weekIndex) => {
      ...
      const parts = jalaliMonthFmt.formatToParts(parseISO(firstActivity.date))
      const month = parts.find((part) => part.type === "month")?.value ?? ""
      const jYear = parts.find((part) => part.type === "year")?.value ?? ""

      const prevLabel = labels.at(-1)
      const yearChanged = prevYear !== null && prevYear !== jYear
      prevYear = jYear

      const label = weekIndex === 0 || yearChanged ? `${month} ${jYear}` : month

      if (weekIndex === 0 || !prevLabel || prevLabel.label !== label) {
        return labels.concat({ weekIndex, label })
      }
